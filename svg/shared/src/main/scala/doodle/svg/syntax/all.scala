...
import doodle.svg.algebra.Attributed
...
trait all {
  extension [Alg <: Algebra, A](picture: Picture[Alg, A]) {
    ...
    def attribute[Builder, T](
        attr: AttrPair[Builder, T]
    ): Picture[Alg & Attributed[Builder], A] =
      new Picture[Alg & Attributed[Builder], A] {
        def apply(implicit
            algebra: Alg & Attributed[Builder]
        ): algebra.Drawing[A] =
          algebra.attribute(picture.apply(algebra), attr)
      }
  }
}
