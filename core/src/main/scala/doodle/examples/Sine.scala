...
object Sine {
  ...
  def errorPlot(data: List[Point], periods: List[Int]): Image = {
    val errors = periods.sorted.map(p => Point(p.toDouble, error(sine(2000, 150, p), data)))
 ...
 // (scaled.map(pt => Image.circle(13.0).at(pt.toVec).fillColor(Color.white).noStroke)).allOn
  ...
  def styledSine(curve: AngularCurve): Image =
    interpolatingSpline(curve.sample(200)).strokeWidth(11.0).strokeColor(Color.white)

  val spacer = Image.rectangle(2000, 500).noStroke.noFill

  val sines = {
    val s1 = interpolatingSpline(sine(2000, 150, 200).sample(200)).strokeWidth(11.0)
    val s2 = interpolatingSpline(sine(2000, 150, 400).sample(200)).strokeWidth(11.0)
    val s3 = interpolatingSpline(sine(2000, 150, 800).sample(200)).strokeWidth(11.0)
  ...
  val images =
    samples.map{ data =>
      ...
      val dataPlot =
        (data.map(pt => Image.circle(9).at(pt.toVec)))
          .allOn
          .fillColor(underBlue)
          .noStroke
