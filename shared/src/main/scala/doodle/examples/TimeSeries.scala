...
object TimeSeries {
  def ballOnStick(length: Int, ballColor: Color, stickColor: Color): Image = {
    val ball = Image.circle(10).lineWidth(5).lineColor(ballColor).noFill
    val stick = Image.line(0, length).lineWidth(5).lineColor(stickColor)

    // shift origin back to the base of the ball-on-stick, so alignment is easier
    (ball above stick).at(0, (length + 10) / 2)
  }
  ...
  object resample {
    ...
    val ghost = Image.square(25).noLine.noFill
    val spacer = Image.rectangle(40, 10).noFill.noLine
  ...
  object distribution {
    val input =
      Image.rectangle(600, 120).noLine.fillColor(Color.orange)

    val splitSeries =
      Image.rectangle(150, 120).noLine

    def color(hue: Angle): Color =
      Color.hsl(hue, 1.0.normalized, 0.5.normalized)

    val fullSplit =
      splitSeries.fillColor(color(0.degrees)).
    ...
    val spacer = Image.rectangle(150, 120).noLine.noFill
    def processed(hue: Angle): Image =
      Image.star(5, 60, 30, 30.degrees).noLine.fillColor(color(hue)).on(spacer)
